...
if args.baseline == 'differential_gcn':
    ...
    model = NDCN(input_size=input_size, hidden_size=hidden_size, A=OM, num_classes=num_classes,
                 dropout=dropout, no_graph=False, no_control=False,
    ...
    OM = torch.FloatTensor(zipf_smoothing(A.numpy()))
    OM = OM.to(device)
    model = TemporalGCN(input_size, 2, input_n_graph, hidden_size_rnn, OM, dropout= dropout, rnn_type='lstm')
    ...
    OM = torch.FloatTensor(zipf_smoothing(A.numpy()))
    OM = OM.to(device)
    model = TemporalGCN(input_size, 2, input_n_graph, hidden_size_rnn, OM, dropout= dropout, rnn_type='gru')
    ...
    OM = torch.FloatTensor(zipf_smoothing(A.numpy()))
    OM = OM.to(device)
    model = TemporalGCN(input_size, 2, input_n_graph, hidden_size_rnn, OM, dropout= dropout, rnn_type='rnn')
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        #print('Iter {:04d}| Train Loss {:.6f}| Relative Train Loss {:.6f} | Test Loss {:.6f} | Relative Test Loss {:.6f} | Time {:.4f}'
        #    .format(itr, loss_train.item(), relative_loss_train.item(),
        #            loss.item(), relative_loss.item(), time.time() - t_start))
